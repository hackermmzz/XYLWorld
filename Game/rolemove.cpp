...
void CorrectRolePosition()
{
    ...
    //正z
    for (; y <= y_end; ++y) {
        if (JudgeBlockCanBarry(GetBlock(floor(camera_pos.x), y, floor(camera_pos.z + RoleBox.r)))) {
        ...
        //正x
        if (JudgeBlockCanBarry(GetBlock(floor(camera_pos.x + RoleBox.r), y, floor(camera_pos.z)))) {
